...
void mxWriteByte(uint8_t DATA);
void mxWrite(uint8_t address, uint8_t value);
void mxInit(void);
void mxClear(void);
/*
*This function takes in ASCII string and displays each letter one after the other with some delay
*/
void mxPrintMessage(const char* str, unsigned int delay_ms);
...
size_t counter = 0;
void loop() {
  mxWrite(0x0a, 0x00); //set brightness
  mxPrintMessage("Bartek", 1000);
}
...
void mxWriteByte(uint8_t DATA) {
  ...
  for (int i = 8; i >= 1; i--) {
    ...
    digitalWrite(PIN_DIN, DATA & 0x80);  //Sending leftmost bit, 0x80 is the mask
    DATA = DATA << 1;                    // shift to left
    digitalWrite(PIN_CLK, HIGH);
  }
}
void mxWrite(uint8_t address, uint8_t value) {
  digitalWrite(PIN_CS, LOW);  //Start to listen
  mxWriteByte(address);
  mxWriteByte(value);
  digitalWrite(PIN_CS, HIGH);  //Stops listening
}
void mxInit(void) {
  mxWrite(0x09, 0x00);
  mxWrite(0x0a, 0x00);
